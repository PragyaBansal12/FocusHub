...
export async function uploadMaterial(req, res) {
  try {
    ...
    if (plan === "free") {
      ...
      if (materialCount >= 3) {
        return res.status(403).json({
          success: false,
          message: "Free tier limit reached. Maximum 3 materials allowed.",
        });
      }
      if (req.file.size > 5 * 1024 * 1024) {
        return res.status(403).json({
          success: false,
          message: "File exceeds 5MB limit for free tier.",
        });
      }
    } else {
      if (req.file.size > 25 * 1024 * 1024) {
        return res.status(403).json({
          success: false,
          message: "File exceeds 25MB limit for premium tier.",
        });
      }
    }
    ...
    const combinedTags = [
      ...new Set([...manualTags, ...autoGenerateTags(req.file.originalname)]),
    ];
...
export async function downloadMaterial(req, res) {
  try {
    ...
    console.log("Material found for download:", material);
    ...
    if (!material.fileUrl) {
      return res
        .status(500)
        .json({ success: false, message: "File URL is missing in database" });
    }
